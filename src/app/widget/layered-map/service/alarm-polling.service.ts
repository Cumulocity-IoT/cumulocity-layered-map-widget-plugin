...
export class AlarmPollingService {
  constructor(private alarm: AlarmService, private queryLayerService: QueryLayerService, private inventory: InventoryService) {}
  ...
  private async fetchMatchingAlarms(config: QueryLayerConfig) {
    ...
    while (res.data.length) {
      const ids = res.data.filter((alarm) => !result.has(alarm.source.id)).map((alarm) => alarm.source.id);
