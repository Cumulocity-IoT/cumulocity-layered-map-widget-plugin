...
import { Component, ElementRef, Input, OnInit, OnDestroy, ViewChild, Output, EventEmitter, forwardRef, ChangeDetectionStrategy, ViewEncapsulation } from '@angular/core';
// @ts-ignore
...
export class JsonEditorComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (!this.options.onChangeText && this.textChange) {
      this.options.onChangeText = this.onChangeText.bind(this);
    }
...
export type { JsonEditorOptions, JsonEditorMode, JsonEditorTreeNode, IError };
